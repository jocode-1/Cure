...
private $API_KEY_DEV = '64125agshaii168sdhd4b6bdb5';
...
private $LIVE_URL = 'https://mobilenig.com/API/airtime_premium?';
    ...
    public function resendToken($conn, $account_id)
    {
        $auth = $this->generateAuth();
        $phone = $this->fetchMobileByAccount($conn, $account_id);
        $this->updateAuthCode($conn, $account_id, $auth);
        return $status = $this->sendSms($phone, $auth);
    }
    ...
    public function updateAuthCode($conn, $account_id, $auth)
    {
        $sql = "UPDATE accounts SET `token` = '$auth' WHERE `user_id` = '$account_id'";
        $stmt = mysqli_query($conn, $sql);
    }
    ...
    public function createSavings($conn, $account_id, $savings_name, $savings_amount, $frequency_code, $rate, $start_date, $end_date)
    {
    ...
    public function fetch_account_details($conn, $account_id)
    {
        ...
        return $this->login_users($conn, $account_id, $email, $password, $kyc, $fullname, $pass);
    }
    ...
    public function login_users($conn, $account_id, $email, $password, $kyc, $fullname, $pass)
    {
        $status = "";
        if ($this->validateUser($conn, $email, $password) > 0) {
            ...
            //echo $token;
            $status =  json_encode(array("account_id" => $account_id, "account_name" => $fullname, "email" => $email, "tokenType" => "Bearer", "expiresIn" => "3600", "accessToken" => $token, "kyc" => $kyc, "passActive" => $pass));
        } else {
    ...
    public function login_users_mobile($conn, $email, $password, $firebaseKey)
    {
        $status = "";
        $user_data = $this->fetchAccountId($conn, $email);
        if ($this->validateUserMobile($conn, $email, $password, $firebaseKey) > 0) {
            $userId = $email . $password;
            $secret = 'sec!ReT423*&';
            $expiration = time() + 3600;
            $issuer = 'localhost';
            ...
            if ($active == 'N') {
                ...
                $status =  json_encode(array("responseCode" => "00", "status" => "success", "message" => "active", "account_id" => $account_id, "account_name" => $fullname, "email" => $email, "tokenType" => "Bearer", "expiresIn" => "3600", "accessToken" => $token, "kyc" => $kyc, "passActive" => $pass));
            }
        } else {
    ...
    public function validateUserMobile($conn, $email, $password, $firebaseKey)
    {
        $sql = "SELECT * FROM `accounts` WHERE `email` = '$email' AND `password` = '$password' AND `firebasekey` = '$firebaseKey'";
        $stmt = mysqli_query($conn, $sql);
        $count = mysqli_num_rows($stmt);
        return $count;
    }
    ...
    public function createRichwayAccount($surname, $othernames, $fullname, $phone, $email, $genCode)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://52.168.85.231/BankOneWebAPI/api/Account/CreateAccountQuick/2?authtoken=f0227d19-e169-4f05-845d-25699d33324b',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public function fetchRichwayDetails($customerId)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://52.168.85.231/BankOneWebAPI/api/Account/GetActiveSavingsAccountsByCustomerID2/2?authtoken=f0227d19-e169-4f05-845d-25699d33324b&customerId=' . $customerId,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public function fetch_balance($account_id)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://52.168.85.231/BankOneWebAPI/api/Account/GetAccountByAccountNumber/2?authtoken=f0227d19-e169-4f05-845d-25699d33324b&accountNumber=' . $account_id,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
 ...
 "AuthenticationCode":"f0227d19-e169-4f05-845d-25699d33324b"
    ...
    "Narration": "' . $narration . '",
    "AuthenticationKey": "f0227d19-e169-4f05-845d-25699d33324b"
}',
            CURLOPT_HTTPHEADER => array(
                'Content-Type: application/json'
            ),
    ...
    public function fetchTransactions($account_id)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://52.168.85.231/BankOneWebAPI/api/Account/GetTransactions/2?authtoken=f0227d19-e169-4f05-845d-25699d33324b&accountNumber=' . $account_id . '&fromDate=2022-04-19&toDate=' . $date . '&institutionCode=100640&numberOfItems=10',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public function updateKYC($conn, $account_id)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://52.168.85.231/BankOneWebAPI/api/Account/UpdateAccountTier2/2?authtoken=f0227d19-e169-4f05-845d-25699d33324b',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public function fetchInterBanks()
    {
		...
		public function get_airtime($account,$network,$phone,$amount){
        $status = '';
        
        $transaction_id = substr(str_shuffle(str_repeat("0123456789", 8)), 0, 8);
        $narration = 'MTN AIRTIME-VTU Recharge of NGN'.$amount.' to '.$phone.' #REF'.$transaction_id;
		$curl = curl_init();
	...
	public function pay_cable_bills($vendor,$card_number,$product_code,$customer_name,$price,$account_id, $firebaseKey){
		...
		curl_setopt_array($curl, array(
  		CURLOPT_URL => 'https://mobilenig.com/API/bills/'.$vendor.'?username=BERNINI47&api_key=dc8509a2e9bbebe435c43b38d7243988&smartno='.$card_number.'&product_code='.$product_code.'&customer_name='.$slimName.'&customer_number='.$account_id.'&price='.$price.'&trans_id='.$unique,
  		CURLOPT_RETURNTRANSFER => true,
  		CURLOPT_ENCODING => '',
  		CURLOPT_MAXREDIRS => 10,
  		CURLOPT_TIMEOUT => 0,
